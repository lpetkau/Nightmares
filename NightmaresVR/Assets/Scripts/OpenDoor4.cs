        ...
        if (other.gameObject.tag == "Player")
        {
            if (GameManager.Instance.Door4Locked == false)
            {
                if (Input.GetButton("Interact") && CanInteract == true)
                {
                    ...
                    IsOpen = this.gameObject.GetComponentInParent<Door4>().CheckState();

                    if (IsOpen == false)
                    {
                        //Debug.Log("Open");
                        this.gameObject.GetComponentInParent<Door4>().SetDesiredOpen();
                    ...
                    {
                        //Debug.Log("Close");
                        this.gameObject.GetComponentInParent<Door4>().SetDesiredClosed();
                        Physics.IgnoreCollision(this.GetComponent<BoxCollider>(), other.gameObject.GetComponent<CharacterController>()); // player ignore door
                    }
                }
                else if (CanInteract)

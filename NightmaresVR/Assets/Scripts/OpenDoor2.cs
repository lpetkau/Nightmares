...
public class OpenDoor2 : MonoBehaviour {
    ...
    void OnTriggerStay(Collider other)
    {
        ...
        {
            if (GameManager.Instance.Door2Locked == false)
            {
                if (Input.GetButton("Interact") && CanInteract == true)
                {
                    ...
                    IsOpen = this.gameObject.GetComponentInParent<Door2>().CheckState();

                    if (IsOpen == false)
                    {
                        //Debug.Log("Open");
                        this.gameObject.GetComponentInParent<Door2>().SetDesiredOpen();
                    ...
                    {
                        //Debug.Log("Close");
                        this.gameObject.GetComponentInParent<Door2>().SetDesiredClosed();
                        Physics.IgnoreCollision(this.GetComponent<BoxCollider>(), other.gameObject.GetComponent<CharacterController>()); // player ignore door
                    }
                }
                else if (CanInteract)
